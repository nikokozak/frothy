...
static int test_record_roundtrip_and_truthful_inspection(void) {
  ...
  ok &= expect_binding_view("point", FROTHY_VALUE_CLASS_RECORD, "record",
  ...
  release_value(&value);
  ok &= expect_binding_view("restoredPoint", FROTHY_VALUE_CLASS_RECORD, "record",
                            "Point: 1, 2",
...
static int test_record_cycle_save_rejection(void) {
  ...
  release_value(&value);
  ok &= expect_binding_view("node", FROTHY_VALUE_CLASS_RECORD, "record",
