...
function(froth_embed_board_assets target root board embed)
endfunction()

function(frothy_generate_board_pin_seed target root board)
endfunction()

function(frothy_generate_board_base_lib target root board)
endfunction()
...
if(NOT SMOKE_DEFINITION_SET MATCHES ";FROTH_HEAP_SIZE=8192;")
...
if(NOT SMOKE_DEFINITION_SET MATCHES ";FROTHY_PAYLOAD_CAPACITY=32768;")
