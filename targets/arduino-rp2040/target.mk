...
TARGET_BUILD_COMMAND = set -e; $(ARDUINO_RP2040_TOOLCHAIN_CHECK); test "$(ARDUINO_RP2040_SKETCH_DIR)" != "/arduino-sketch/Frothy"; rm -rf "$(ARDUINO_RP2040_SKETCH_DIR)"; mkdir -p "$(ARDUINO_RP2040_SKETCH_DIR)/src" "$(ARDUINO_RP2040_BUILD_DIR)" "$(ARDUINO_RP2040_OUTPUT_DIR)"; cp "$(ARDUINO_RP2040_SKETCH_TEMPLATE)" "$(ARDUINO_RP2040_SKETCH_DIR)/Frothy.ino"; for source in $(FROTHY_SOURCES); do destination="$(ARDUINO_RP2040_SKETCH_DIR)/src/$$source"; mkdir -p "$$(dirname "$$destination")"; cp "$$source" "$$destination"; done; arduino-cli compile --fqbn "$(BOARD_ARDUINO_FQBN)" --board-options "$(ARDUINO_RP2040_BOARD_OPTIONS)" --build-path "$(ARDUINO_RP2040_BUILD_DIR)" --output-dir "$(ARDUINO_RP2040_OUTPUT_DIR)" --warnings all --build-property 'compiler.c.extra_flags=$(ARDUINO_RP2040_FLAGS)' --build-property 'compiler.cpp.extra_flags=$(ARDUINO_RP2040_FLAGS)' "$(ARDUINO_RP2040_SKETCH_DIR)"
